...
export default function TankCodeMasterPage() {
  ...
  const fetchCodes = async () => {
    setLoading(true);
    try {
      const resp = await api.get('/tank_code_master/');
      if (resp.data.success) {
        setItems(resp.data.data);
      }
    } catch (err) {
      console.error("Error fetching tank codes:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const sortedItems = useMemo(() => {
    ...
    if (filterTerm) {
      filtered = filtered.filter(item =>
        (item.tankcode_iso && item.tankcode_iso.toLowerCase().includes(filterTerm.toLowerCase())) ||
  ...
  const handleAdd = async () => {
    ...
    try {
      const resp = await api.post('/tank_code_master/', { tankcode_iso: trimmed });
      if (resp.data.success) {
        setNewValue('');
        fetchCodes();
      } else {
  ...
  const handleEdit = (item) => {
    ...
    setEditingValue(item.tankcode_iso);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (!trimmed) return;
    try {
      const resp = await api.put(`/tank_code_master/${editingId}`, { tankcode_iso: trimmed });
      if (resp.data.success) {
        setEditingId(null);
        setEditingValue('');
        fetchCodes();
  ...
  const handleToggleStatus = async (item) => {
    ...
    try {
      const resp = await api.put(`/tank_code_master/${item.id}`, { status: newStatus });
      if (resp.data.success) {
        fetchCodes();
      }
    } catch (err) {
    ...
    <div className="flex flex-col flex-1 p-3 bg-gray-50 overflow-hidden h-full">
      <div className="bg-white rounded shadow-sm overflow-hidden flex flex-col h-full border border-gray-200">
        ...
        <div className="p-4 border-b border-gray-100 bg-white">
          <div className="mb-4">
            <h1 className="text-2xl font-bold text-[#546E7A]">Tank Code / ISO Code Master</h1>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                <option value="name">Tank Code / ISO Code</option>
            ...
            {/* Add Row */}
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-base font-semibold text-gray-700 min-w-[80px]">Add Code</span>
              <div className="flex-grow max-w-xs">
                <input
                  type="text"
                  placeholder="Enter Tank Code to add..."
                  value={newValue}
          ...
          <table className="w-full border-collapse">
            <thead className="sticky top-0 bg-[#455A64] text-white shadow-sm z-10">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-bold uppercase tracking-wider border-b border-[#37474F]">Tank Code / ISO Code</th>
