...
export default function TankGaugeTab({ tankId, onNext, onClose }) {
    ...
    const loadGauges = async () => {
        try {
            ...
            // 1. Fetch master gauges list
            const masterData = await getAllMasterData();
            const gaugeMasterList = masterData.master_gauges || [];
            const gaugeNames = gaugeMasterList.map(v => v.name);

            // 2. Fetch existing tank gauges
            const res = await api.get(`/tank-gauges/tank/${tankId}`);
            const responseData = res.data?.data || res.data;
            const dataArray = (responseData && Array.isArray(responseData.gauges))
                ? responseData.gauges
                : (Array.isArray(responseData) ? responseData : []);

            console.log("Loaded gauges for tank:", tankId, dataArray);
            ...
            if (dataArray.length > 0) {
                ...
                initialized = gaugeNames.map(f => ({
                    id: null,
                    feature: f,
                    status_id: 1
                }));
            }
            setGauges(initialized);
        } catch (err) {
    ...
    const handleSave = async () => {
        try {
            ...
            const payloadGauges = gauges.map(v => ({
                id: v.id,
                feature: v.feature,
                status_id: v.status_id
            }));
            ...
            const payload = {
                tank_id: tankId,
                gauges: payloadGauges
            };

            if (hasExistingData) {
                await api.post('/tank-gauges/update', payload);
            } else {
                await api.post('/tank-gauges/create', payload);
                setHasExistingData(true);
            }
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                    {[0, 1, 2].map(colIndex => (
                        <div key={colIndex} className="flex flex-col border border-gray-200 rounded-sm">
                            {gauges.filter((_, i) => i % 3 === colIndex).map((v, i) => {
